...
module.exports = function () {
    ...
    function* onPacket({ ecs }) {
        ...
        //Les EC (Thèses et sujets de thèses) sont d'un autre rctype que BIO, d'après le parser theses.fr
        const unitidsThesesSujets = ecs.filter(([ec, done]) => ec.rtype != 'BIO').map(([ec, done]) => ec.unitid);

        //Les personnes sont de rctype BIO
        const unitidsPersons = ecs.filter(([ec, done]) => ec.rtype == 'BIO').map(([ec, done]) => ec.unitid);

        const maxAttempts = 5;
        let tries = 0;
        let docsThesesSujets;

        while (!docsThesesSujets) {
            ...
            try {
                docsThesesSujets = yield queryThesesSujets(unitidsThesesSujets); //requête API theses
            } catch (e) {
        ...
        while (!docsPersons) {
            ...
            try {
                docsPersons = yield queryPersons(unitidsPersons); //requête API Personnes
            } catch (e) {
                logger.error(`Thesesfr: ${e.message}`);
            }
        ...
        docsThesesSujets.forEach(doc => {
            if (doc && doc.id) {
                docResults.set(doc.id, doc);
            }
        });

        docsPersons.forEach(doc => {
            if (doc && doc.id) {
                docResults.set(doc.id, doc);
            }
        });
    ...
    function enrichEc(ec, result) {
        ...
        //Si il s'agit d'une Personne (PPN) rctype égale à BIO
        if (ec.rtype == 'BIO') {
            ...
            //Pour un organisme : organismeN et organismePpn
        }
        //Sinon, il s'agit d'une thèse ou d'un sujet de thèse (rctype autre que BIO)
        else {
            //Thèse soutenue
            if (result.nnt) {
                ec['nnt'] = result.nnt;
            ...
            logger.verbose('Thèse ou sujet de thèse, etab Soutenance ==> ' + ec['nnt'] + ' ' +ec['etabSoutenanceN']);
        }
    }

    /**
     * Request metadata from ThesesFr API for given Thesis or ongoing Thesis IDs
     * @param {Array} unitids the ids to query
     */
    function queryThesesSujets(unitids) {
        ...
        unitids.forEach(id => {
            /^(([0-9]{4})([a-z]{4})[0-9a-z]+)$/i.test(id) ? nnts.push(id) : numSujets.push(id);
        });
        ...
        return new Promise((resolve, reject) => {
            const options = {
                method: 'GET',
                json: true,
                uri: `https://theses.fr/api/v1/theses/recherche/${query}`
            };
    ...
    /**
     * Request metadata from ThesesFr API for given Persons IDs
     * @param {Array} unitids the ids to query
     */
    function queryPersons(unitids) {
        report.inc('thesesfr', 'thesesfr-queries');
        ...
        return new Promise((resolve, reject) => {
            const options = {
                method: 'GET',
                json: true,
                uri: `https://theses.fr/api/v1/personnes/recherche/${query}`
            };
